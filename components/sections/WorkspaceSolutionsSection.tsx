        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {solutions.map((solution, index) => (
            <motion.div
              ...
              className="text-center space-y-4 p-8 theme-bg-secondary rounded-lg hover:bg-cyan-400 hover:text-white transition-all duration-300 cursor-pointer group relative overflow-hidden"
              style={{ transformStyle: 'preserve-3d' }}
            >
              {/* Background gradient on hover */}
              <motion.div
                className="absolute inset-0 bg-gradient-to-br from-cyan-400/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                initial={{ scale: 0 }}
                whileHover={{ scale: 1 }}
